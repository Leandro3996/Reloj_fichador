...
IF "%1"=="" (
    echo ======================================================
    echo      ERROR: Debes especificar un test para ejecutar
    echo ======================================================
    echo.
    echo Uso: run_specific_test.bat [ruta_del_test]
    echo.
    echo Ejemplos:
    echo   run_specific_test.bat apps.reloj_fichador.tests.test_views
    echo   run_specific_test.bat apps.reloj_fichador.tests.test_models.OperarioModelTest
    echo.
    goto :end
)
...
REM Ejecutar las pruebas con la configuración de SQLite
echo Ejecutando pruebas: %1
docker-compose exec web python -m django test %1 -v 2 --settings=tests_utils.sqlite_settings
